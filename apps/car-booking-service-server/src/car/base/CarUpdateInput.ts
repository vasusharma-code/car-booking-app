/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { BookingUpdateManyWithoutCarsInput } from "./BookingUpdateManyWithoutCarsInput";
import {
  ValidateNested,
  IsOptional,
  IsString,
  MaxLength,
  IsNumber,
  Min,
  Max,
  IsInt,
} from "class-validator";
import { Type } from "class-transformer";
import { ReviewUpdateManyWithoutCarsInput } from "./ReviewUpdateManyWithoutCarsInput";

@InputType()
class CarUpdateInput {
  @ApiProperty({
    required: false,
    type: () => BookingUpdateManyWithoutCarsInput,
  })
  @ValidateNested()
  @Type(() => BookingUpdateManyWithoutCarsInput)
  @IsOptional()
  @Field(() => BookingUpdateManyWithoutCarsInput, {
    nullable: true,
  })
  bookings?: BookingUpdateManyWithoutCarsInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  make?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  model?: string | null;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsNumber()
  @Min(-999999999)
  @Max(999999999)
  @IsOptional()
  @Field(() => Number, {
    nullable: true,
  })
  pricePerDay?: number | null;

  @ApiProperty({
    required: false,
    type: () => ReviewUpdateManyWithoutCarsInput,
  })
  @ValidateNested()
  @Type(() => ReviewUpdateManyWithoutCarsInput)
  @IsOptional()
  @Field(() => ReviewUpdateManyWithoutCarsInput, {
    nullable: true,
  })
  reviews?: ReviewUpdateManyWithoutCarsInput;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsInt()
  @Min(-999999999)
  @Max(999999999)
  @IsOptional()
  @Field(() => Number, {
    nullable: true,
  })
  year?: number | null;
}

export { CarUpdateInput as CarUpdateInput };
